...
class Game {
  ...
  private async AppTitle(): Promise<void> {
    ...
    private async App():Promise<void>{
        ...
        // Game variables
        let enemies:string[]=["Skeleton","Zombie","Warrior","Assasain"];
        ...
        let healthPortionDropChance:number=50; //percentage
        ...
        Game:while (running) {
            ...
            let enemyHealth:number=randomNo(maxEnemyHealth);
            // randomly choosing an enemy from the enemy list
            let enemy:string=enemies[randomNo(enemies.length)];
            ...
            while(enemyHealth>0){
                ...
                // break;
                if (choice=="1") {
                    let damageDealt:number=randomNo(attackDamage);//damage to enemy
                    let damageTaken:number=randomNo(enemyAttackDamage);//damage to usr
